...
"Containers" is the **chest** (3.5c), read through `Chest.storage()` — a live `Inventory`, so the query walks the same model as the player's own and the 3.6a UI binds the same `slot_changed` ([automation.md](automation.md) §Categories → Utility, which owns how a container is *found*). The query itself lands at 3.6b, at a radius of ~12 tiles — the reach radius is 4.5 and would make you stand on the chest.

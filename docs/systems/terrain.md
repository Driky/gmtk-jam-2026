...
**Torch = a one-cell entity scene** (`scenes/torch.tscn`, `scripts/terrain/torch.gd`), registered in the terrain entity dict so nothing else can claim the cell, and found by the grid through the `light_source` group. It owns **no light node and no visibility notifier** — there are no per-light budgets left to manage. Placement and pick-up are [player-combat.md](player-combat.md) §Placement's.

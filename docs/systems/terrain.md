...
**Budget — measured in-browser, not estimated.** Full-res (112×77 = 8,624 cells at 1× zoom, the worst case since zoom > 1 shows *less* world): **sample 1.4 ms · solve 7.0 ms · upload 0.6 ms**. Half-res is 2.0 ms total. **wasm runs this at ~1.0× desktop speed** — there is no web penalty, because the solve is flat `PackedFloat32Array` arithmetic rather than the heap/`Dictionary`/native-call mix that makes the flow field cost 57 ms ([enemies.md](enemies.md)). Fallback ladder if it ever matters: amortize the eight sweeps across frames → solve at 10–15 Hz instead of per frame → half-res grid (1 sample per 2×2 tiles; bilinear hides it) → drop `PASSES` to 1. **The vicinity light cap and the off-screen light disable are deleted, not deferred** — both existed solely to manage `PointLight2D` limits (the ~15-lights-per-`CanvasItem` ceiling and per-light shadow passes) that a grid does not have. The toast for *buffer-zone* rejection survives, since that rule is about placement, not light ([ui.md](ui.md)).

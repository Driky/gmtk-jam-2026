...
**Torch = a one-cell `Deployable`** (`scenes/torch.tscn`, `scripts/terrain/torch.gd`), registered in the terrain entity dict so nothing else can claim the cell, and found by the grid through the `light_source` group. It owns **no light node and no visibility notifier** — there are no per-light budgets left to manage. Placement is [player-combat.md](player-combat.md) §Placement's; removal is its §Un-deploying (LMB, hit-counted, mob-shielded).
- **3.1 folded it onto the `Deployable` base** ([automation.md](automation.md)), which owns everything structural — W×H footprint, HP, the support bitmask, `on_placed`/`on_removed`, and the one `pop_to_pickup` drop path. **The script here is down to `extends Deployable` plus `light_color`**, and the numbers (`item_id`, `max_hp = 20`) are authored in the scene. The torch keeps this path rather than moving under `scripts/automation/` because the repo convention is script path ≈ owning doc: automation.md owns the base, this doc owns the torch *as a light source*.
- ❗️**It now has `current_hp`, and that is the visible behaviour change.** `flow_field.gd` reads `ent.get("current_hp")`, so a torch costs the field `hp × ENTITY_HP_COST_FACTOR` — but at 0.01 against a 1.0 move cost, a 20 HP torch adds **0.2**, a fifth of one step, and reroutes essentially nothing. What actually changes is that **mobs stop and hit it**: a walker at 8 damage / 0.8 s takes 3 hits ≈ **1.6 s per torch passed**, so a lit tunnel is an accidental mob-slowing corridor. `max_hp` is the tuning knob. If torches turn out too sticky the escape hatch is one exported `blocks_mobs := false` checked in `Enemy._attackable_entity` — **don't add it pre-emptively.**
- **Accepted, not solved:** mining the tile a torch is mounted on leaves it floating. The engine is safe (`damage_tile` on air returns false, and `set_tile`'s entity assert cannot fire because `can_place_at` already rejects an occupied cell), so it is cosmetic only. The support rule that replaces this is [automation.md](automation.md)'s, landing later in 3.1.

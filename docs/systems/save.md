...
- **Entities:** deployables (type id, pos, rotation, inventories, internal state e.g. craft progress), conveyor slot contents (stacks), loot bags.
  - ⚠️ **`Deployable`'s yield credit (3.7) is deliberately NOT serialized.** It is the fraction of an item a yield buff still owes a machine ([automation.md](automation.md) §Deployable base), so dropping it costs at most one item per machine across a reload — not worth a save field, and worth saying out loud so it does not look like an omission.

...
**Scene placeables (2.7):** `ItemStats.place_scene` — non-null makes RMB instance that **scene** into the cell instead of writing a tile, and it **wins over the material-id path**. Data-driven on purpose, following the `hitbox_scene` precedent: Day 3's miner, conveyor and turret ([automation.md](automation.md)) become `.tres` rows rather than new branches in the player. The scene must expose `setup(cell)` / `register(terrain) -> bool` / `pick_up(terrain)`. Parent is the player's own parent (`Main`), matching `_drop_loot_bag`.

**Validity is `can_place_at` verbatim**, not a relaxed copy — the cardinal-adjacency rule isn't a block-shaped compromise a torch inherits, it *is* the rule you want: no torches floating in mid-air, mount them on a wall.

❗️**Order is the reverse of the block path, and has to be.** `set_tile` cannot fail, so blocks consume the item and then write. `register` *can* fail, so a scene claims the cell first and the item is consumed only once the claim succeeds — a failed claim hands the item back instead of eating it.
...
**Counted in HITS, not accumulated damage** (torch = 1; heavier 3.1 machines raise their own `REMOVAL_HITS`). A swing is a discrete beat on the item's cooldown, so "three hits" is something a player can feel and count, where a hardness/second curve is not. The counter lives on the deployable, so each one owns its own toughness.
...
A completed removal **pops the item out as a world pickup** (consistent with mining a tile) carrying `grants_xp = false` — otherwise place → remove → place is an infinite looting-XP loop ([progression.md](progression.md)). The cell is freed **eagerly** rather than on `queue_free`, which defers to end of frame and would otherwise reject a re-place into the same cell on the same tick.

**`as Torch`, not a group check**, is what keeps the Core un-removable with no special case anywhere — and is exactly what 3.1 generalises to `as Deployable`.

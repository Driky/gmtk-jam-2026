...
- [x] `SkillNode` Resources (prereqs, costs, leveled buffs) + 13 nodes in three branches; buffs live through `get_stat`. ⚠️ **`unlock_recipes[]` is struck**: the recipe row's `unlocked_by` is the single source of that edge, since storing it on both sides is one fact with two writable copies.
- [x] The three buff seams that **did not exist** — `crafting_speed`, `crafting_yield`, `resource_yield` are named in [progression.md](systems/progression.md) and read by nothing. ❗️Both yields are **FACTORY buffs, on deployables only**: the miner and the crafting station, never the player's own dig or a hand craft — `spawn_at` is also `pop_to_pickup`'s drop path, so a multiplier there returns two turrets for one. ⚠️ A fractional multiplier on an integer output needs a deterministic answer: an accumulator **per machine** on `Deployable`, beside the `spend_power_tick` idiom it copies.
- [x] The tree tab — a **third tab in 3.6a's existing window**, not a screen of its own, and the step that fills in the `unlocked_by` column 3.6b ships empty on every recipe row. It also surfaces the upgrade-point count [ui.md](systems/ui.md) §HUD defers to exactly here.
- [x] ❗️**The step that decided what a new run can build**: everything but the pickaxe, the ladder and the torch goes behind a point, so obtainability moves from 3.6b's bootstrap-recipe rule to the tree's roots — pinned by tests, not by playing it once.
...
- ⚠️ **The last clause got sharper at 3.7, and it is now the ONLY way in.** A fresh run can hand-craft a pickaxe, a ladder and torches; everything else — including the miner, the conveyor and the inserter the first clause names — is behind a node. So "a skill point spent unlocks a recipe" is not a demo of the tab, it is the precondition for the rest of the line ([progression.md](systems/progression.md) §Recipe tiers).
